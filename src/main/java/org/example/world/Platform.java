<<<<<<< HEAD
    ...
    // import java.awt.GradientPaint; // Removido - não utilizado
    ...
    public class Platform {
        ...
        /**
         * Renderização cyberpunk com efeitos neon
         */
        private void drawFallback(Graphics2D g2d) {
            Color platformColor = getPlatformColor();
            Color borderColor = getBorderColor();

            // Efeito de brilho neon (halo)
            g2d.setColor(new Color(borderColor.getRed(), borderColor.getGreen(), borderColor.getBlue(), 30));
            ...
            drawCyberpunkEffects(g2d);
        }

        /**
         * Desenha efeitos cyberpunk específicos para cada tipo de plataforma
         */
        private void drawCyberpunkEffects(Graphics2D g2d) {
            Color borderColor = getBorderColor();

            switch (type) {
                case MOVING:
                    // Linhas de energia pulsantes
                    g2d.setColor(new Color(borderColor.getRed(), borderColor.getGreen(), borderColor.getBlue(), 100));
                    for (int i = 0; i < width; i += 20) {
                        g2d.drawLine(x + i, y + 2, x + i, y + height - 2);
                    }
                    break;

                case BOUNCY:
                    // Círculos de energia
                    g2d.setColor(new Color(borderColor.getRed(), borderColor.getGreen(), borderColor.getBlue(), 80));
                    g2d.drawOval(x - 3, y - 3, width + 6, height + 6);
                    break;

                case ICE:
                    // Padrão de cristais
                    g2d.setColor(new Color(borderColor.getRed(), borderColor.getGreen(), borderColor.getBlue(), 60));
                    for (int i = 0; i < width; i += 15) {
                        g2d.drawLine(x + i, y, x + i + 8, y + height);
                    }
                    break;
                ...
                case ONE_WAY:
                    ...
                    for (int i = 0; i < width; i += 25) {
                        int[] arrowX = {x + i, x + i + 10, x + i + 5};
                        int[] arrowY = {y + height/2, y + height/2, y + height/2 + 8};
                        g2d.fillPolygon(arrowX, arrowY, 3);
                    }
                    break;

                default:
                    // Padrão de circuitos para plataformas normais
                    g2d.setColor(new Color(borderColor.getRed(), borderColor.getGreen(), borderColor.getBlue(), 40));
                    for (int i = 0; i < width; i += 30) {
                        g2d.drawLine(x + i, y + 5, x + i + 15, y + height - 5);
                    }
                    break;
            }
        }
        ...
        private Color getPlatformColor() {
            ...
            switch (type) {
                case GROUND: return new Color(20, 20, 40);       // Azul escuro cyberpunk
                case BRICK: return new Color(40, 40, 80);        // Azul médio
                case PIPE: return new Color(0, 100, 150);        // Azul neon
                case CLOUD: return new Color(100, 0, 150);       // Roxo neon
                case MOVING: return new Color(0, 255, 255);      // Ciano brilhante
                case BREAKABLE: return new Color(150, 0, 100);   // Magenta escuro
                case ICE: return new Color(0, 200, 255);         // Azul gelo neon
                case BOUNCY: return new Color(255, 0, 150);      // Rosa neon
                case ONE_WAY: return new Color(0, 255, 100);     // Verde neon
                default: return new Color(30, 30, 60);           // Cinza cyberpunk
            }
        }
        ...
        private Color getInfinitePhasePurpleColor() {
            switch (type) {
                case GROUND: return new Color(75, 0, 130);       // Indigo profundo
        ...
        private Color getBorderColor() {
            ...
            switch (type) {
                case GROUND: return new Color(0, 255, 255);      // Ciano neon
                case BRICK: return new Color(0, 200, 255);       // Azul neon
                case PIPE: return new Color(0, 255, 200);        // Verde água neon
                case CLOUD: return new Color(255, 0, 255);       // Magenta neon
                case MOVING: return new Color(255, 255, 0);      // Amarelo neon
                case BREAKABLE: return new Color(255, 0, 150);   // Rosa neon
                case ICE: return new Color(0, 255, 255);         // Ciano brilhante
                case BOUNCY: return new Color(255, 100, 255);    // Rosa magenta
                case ONE_WAY: return new Color(0, 255, 100);     // Verde neon
                default: return new Color(100, 100, 255);        // Azul neon
            }
        }
        ...
        private Color getInfinitePhasePurpleBorder() {
            switch (type) {
                case GROUND: return new Color(25, 0, 51);        // Indigo muito escuro
                case BRICK: return new Color(75, 0, 130);        // Indigo
        ...
        private void drawPlatformEffects(Graphics2D g2d) {
            // EFEITOS DESABILITADOS - VISUAL LIMPO SEM QUADRADOS EXTRAS
            // Todos os efeitos visuais foram removidos para eliminar elementos desnecessários
        }
        ...
        /**
         * Adiciona textura de superfície à plataforma
         */
        private void drawSurfaceTexture(Graphics2D g2d) {
            // Adicionar pequenos detalhes de textura
            g2d.setColor(new Color(255, 255, 255, 30));
            g2d.setStroke(new BasicStroke(1));

            // Linhas horizontais de textura
            for (int i = 0; i < height; i += 8) {
                g2d.drawLine(x + 2, y + i, x + width - 2, y + i);
            }

            // Linhas verticais de textura
